...
async fn main() -> anyhow::Result<()> {
  ...
  let mut subject_properties = Object::new();
  /*
  ...
  subject_properties.insert(
    "id".to_string(),
    serde_json::Value::String(String::from(alice_document.id().as_str())),
  ); */
  // Create a credential subject indicating the degree earned by Alice.
  // let subject: Subject = Subject::with_id_and_properties(Url::parse(alice_document.id().as_str())?, subject_properties);

  let credData = CredentialData {
    // validFrom: nowTs,
    // issuanceDate: nowTs,
    // issuer: Url::parse(issuer_document.id().as_str())?,
    // credentialSubject: subject_properties,
